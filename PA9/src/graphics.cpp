...
bool Graphics::Initialize(int width, int height)
{
  ...
  if (m_modelMatrix == INVALID_UNIFORM_LOCATION) 
  {
   ...
   glLightModelfv( GL_LIGHT_MODEL_AMBIENT, lightmode);
...
void Graphics::Render()
{
  ...
  glUniform4fv(m_LightPosition,1,glm::value_ptr(glm::vec4(1000.0, 1000.0, 0.0, 0.0)));
  glUniformMatrix4fv(m_Projection, 1, GL_FALSE, glm::value_ptr(m_camera->GetProjection()));
  glUniform4fv(m_AmbientProduct,1,glm::value_ptr(glm::vec4(0.0, 0.0, 0.0, 1.0)));
  ...
  //glUniform1f(m_Shininess,glm::value_ptr(glm::vec1(50.0)));
